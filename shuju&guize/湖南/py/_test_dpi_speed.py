...
for dpi in [0.5, 0.75, 1.0]:
    ...
    for idx in range(doc.page_count):
        ...
        tmp = Path(f"_speed_test_{dpi}_{idx:03d}.png")
        pix.save(tmp)
    ...
    print(f"DPI={dpi}: {elapsed:.1f}s, {total} points")
    for t in tables:
        print(f"  {t['title']}: {t['points']}")
    print(f"  每页: {elapsed/doc.page_count:.1f}s")
