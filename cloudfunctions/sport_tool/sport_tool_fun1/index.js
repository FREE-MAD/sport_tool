...
async function findRecentDuplicateRecord(openid, submitMeta) {
  // 本轮“云函数二次校验防重”先停用，只保留前端 globalSubmit.js 拦截。
  // 如需恢复服务端防重，可回滚到基于 openid + submitFingerprint 的最近记录查询逻辑。
  return null;
}
...
function getMainSubCode(code) {
  const text = String(code || '');
  if (text.length < 3) return '';
  return text.slice(-3);
}

function getComputationScopeSet(computationRule) {
  return new Set(Array.isArray(computationRule && computationRule.scopeKeys) ? computationRule.scopeKeys : []);
}

function getMainItemWeight(item, computationRule) {
  const scopeWeights = (computationRule && computationRule.scopeWeights) || {};
  const mainProjectWeights = (computationRule && computationRule.mainProjectWeights) || {};
  const itemWeights = mainProjectWeights.itemWeights || {};
  const subCode = getMainSubCode(item && item.code);

  if (subCode && Object.prototype.hasOwnProperty.call(itemWeights, subCode)) {
    return normalizeWeight(itemWeights[subCode], 1);
  }
  if (subCode && Object.keys(itemWeights).length > 0) {
    return 0;
  }
  return normalizeWeight(scopeWeights.mainProject, 1);
}

function getSpecialItemWeight(item, computationRule) {
  const scopeWeights = (computationRule && computationRule.scopeWeights) || {};
  const scopeKey = item && item.scopeKey;
  const scopeSet = getComputationScopeSet(computationRule);

  if (!scopeKey) return 1;
  if (!scopeSet.has(scopeKey)) return 0;
  return normalizeWeight(scopeWeights[scopeKey], 1);
}

function applyComputationRule(mainScores, specialScores, computationRule) {
  const hasRule = !!(computationRule && computationRule.code);
  if (!hasRule) {
    const fallbackMainScores = (mainScores || []).map((item) => ({
      ...item,
      countedScore: roundToTwo(item.rawScore)
    }));
    const fallbackSpecialScores = (specialScores || []).map((item) => ({
      ...item,
      countedScore: roundToTwo(item.rawScore)
    }));
    const fallbackTotalScore = roundToTwo(
      [...fallbackMainScores, ...fallbackSpecialScores].reduce((sum, item) => sum + (item.countedScore || 0), 0)
    );
    return {
      mainScores: fallbackMainScores,
      specialScores: fallbackSpecialScores,
      totalScore: fallbackTotalScore
    };
  }

  const nextMainScores = (mainScores || []).map((item) => {
    const weight = getMainItemWeight(item, computationRule);
    return {
      ...item,
      weight,
      countedScore: roundToTwo((item.rawScore || 0) * weight)
    };
  });

  const nextSpecialScores = (specialScores || []).map((item) => {
    const weight = getSpecialItemWeight(item, computationRule);
    return {
      ...item,
      weight,
      countedScore: roundToTwo((item.rawScore || 0) * weight)
  ...
  const totalScore = roundToTwo(
    [...nextMainScores, ...nextSpecialScores].reduce((sum, item) => sum + (item.countedScore || 0), 0)
  );

  return {
    mainScores: nextMainScores,
    specialScores: nextSpecialScores,
    totalScore
  };
}
 ...
 * 对所有项目进行评分
 */
async function scoreAll(mainData, specialDataList, genderCode, collectionName, computationRule) {
  ...
  const computedScores = applyComputationRule(mainScores, specialScores, computationRule);

  console.log(
    '[评分] 总分:',
    computedScores.totalScore,
    '| 计分规则:',
    (computationRule && computationRule.code) || 'default',
    '| 基本素质计分:',
    computedScores.mainScores.map(s => s.countedScore),
    '| 专项/辅项计分:',
    computedScores.specialScores.map(s => ({ code: s.code, scopeKey: s.scopeKey, countedScore: s.countedScore, weight: s.weight }))
  );
...
exports.main = async (event, context) => {
  ...
  const resolvedOpenid = clientOpenid || wxContext.OPENID || '';
  const submitMeta = __submitMeta || {};
  ...
  console.log('openid:', resolvedOpenid || 'missing');
  console.log('提交时间:', submitMeta.clientSubmitAt || 'missing', '| 请求ID:', submitMeta.clientRequestId || 'missing');
  console.log('提交流转页:', submitMeta.submitPageRoute || 'unknown', '| 锁Key:', submitMeta.submitLockKey || 'unknown');
  console.log('提交指纹:', submitMeta.submitFingerprint || 'missing', '| 防重窗口ms:', submitMeta.duplicateWindowMs || 'default');
  console.log('环境:', runtimeEnvVersion || 'unknown', '| 评分集合:', scoreCollectionName);
  console.log('规则码:', JSON.stringify({
    computationRuleCode: computationRuleCode || '',
    totalScoreCode: totalScoreCode || '',
    specialCaseCodes: specialCaseCodes || []
  }));
  console.log('计算规则:', JSON.stringify(computationRule || null));
  console.log('总分规则:', JSON.stringify(totalScoreRule || null));
  ...
  try {
    const duplicateRecord = await findRecentDuplicateRecord(resolvedOpenid, submitMeta);
    if (duplicateRecord) {
      console.warn('[防重] 命中重复提交，直接返回上次结果:', duplicateRecord._id, submitMeta.submitFingerprint || submitMeta.clientRequestId);
      return {
        success: true,
        duplicateBlocked: true,
        message: '请勿重复提交，已返回上次结果',
        id: duplicateRecord._id,
        score: {
          mainScores: duplicateRecord.mainScores || [],
          specialScores: duplicateRecord.specialScores || [],
          totalScore: duplicateRecord.totalScore || 0,
        },
      };
    }
    ...
    // ==================== 评分 ====================
    console.log('========== 开始评分 ==========');
    const score = await scoreAll(mainData, specialDataList || [], genderCode, scoreCollectionName, computationRule || null);
    ...
    const result = await db.collection('tool_records').add({
      data: {
        ...
        clientRequestId: submitMeta.clientRequestId || '',
        clientSubmitAt: submitMeta.clientSubmitAt || '',
        clientSubmitTimestamp: submitMeta.clientSubmitTimestamp || 0,
        submitPageRoute: submitMeta.submitPageRoute || '',
        submitLockKey: submitMeta.submitLockKey || '',
        submitFingerprint: submitMeta.submitFingerprint || '',
        duplicateWindowMs: submitMeta.duplicateWindowMs || 5000,
        province,
        provinceCode,
        computationRuleCode: computationRuleCode || '',
        totalScoreCode: totalScoreCode || '',
        ...
        totalScoreRule: totalScoreRule || null,
        specialCaseRules: specialCaseRules || [],
        gender,
        genderCode,
        mainType,
        mainTypeKey,
